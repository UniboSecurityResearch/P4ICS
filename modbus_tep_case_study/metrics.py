...
def _align(ypred, ytrue):
    ypred = np.asarray(ypred).astype(int).ravel()
    ytrue = np.asarray(ytrue).astype(int).ravel()
    n = min(len(ypred), len(ytrue))
    return ypred[:n], ytrue[:n]

def _confusion_counts(ypred, ytrue):
    ypred, ytrue = _align(ypred, ytrue)
    tp = int(np.sum((ypred == 1) & (ytrue == 1)))
    tn = int(np.sum((ypred == 0) & (ytrue == 0)))
    fp = int(np.sum((ypred == 1) & (ytrue == 0)))
    fn = int(np.sum((ypred == 0) & (ytrue == 1)))
    return tp, tn, fp, fn

def accuracy(ypred, ytrue):
    ypred, ytrue = _align(ypred, ytrue)
    return float(np.mean(ypred == ytrue)) if len(ypred) else 0.0

def precision(ypred, ytrue):
    tp, tn, fp, fn = _confusion_counts(ypred, ytrue)
    denom = tp + fp
...
def fb_score(ypred, ytrue, beta=1):
    ...
    denom = (beta**2)*p + r
    return float((1 + beta**2) * p * r / denom) if denom > 0 else 0.0
