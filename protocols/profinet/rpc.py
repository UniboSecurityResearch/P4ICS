...
class RPCCon:
    ...
    def _create_rpc(self, operation, nrd):
        return PNRPCHeader(
            ...
            1, # InterfaceVersion
            0, # SequenceNumber
            operation, #es. PNRPCHeader.CONNECT
    ...
    def connect(self, src_mac=None):
        ...
        ar = PNARBlockRequest(bytes(block),
            ...
            2,
            bytes(8), 
            payload=bytes("Richiesta Connessione", encoding="utf-8")
        )
        nrd = self._create_nrd(ar)
        rpc = self._create_rpc(PNRPCHeader.CONNECT, nrd)
        self.u.sendto(bytes(rpc), self.peer)
        
        data,server_address = self.u.recvfrom(4096)
        print(f"Ricevuto {len(data)} byte da {server_address}")
